...
from tcav_scoring import evaluate_tcav


if __name__ == "__main__":
    # Preprocess images (from TN_2023 raw data)
    input_folder = "TN_2023"
    ...
    LABEL_FILE = "labels.csv"  # Generated by run_preprocessing_pipeline.py
    ...
    for i in Labels:
        for j in example_file:
            ...
            tcav_score = evaluate_tcav(
                biomarker=biomarker_name, image=file_name,
                img_dir=augment_output_folder,
                biomarker_csv=f"{biomarker_name}.csv",
                device=device
            )
            
            Final_Score = (0.4 * similarity_score) + (robustness * 0.1) + (consistency * 0.15) + (threshold * 0.15) + (tcav_score * 0.2)
            print("Final Score is :", Final_Score)
            print(f"  Components: similarity={similarity_score:.4f}, robustness={robustness:.4f}, "
                  f"consistency={consistency:.4f}, threshold={threshold:.4f}, tcav={tcav_score:.4f}")